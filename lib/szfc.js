...
class SzfcClient {
  ...
  async getSaleInfoProListIndex() {
    const res = await superagent.get(URL_SALE_INFO_PROLIST_INDEX).timeout(10000);

    let indexUrl = res.redirects[0];
    logger.debug("current url: ", indexUrl);

    if (!indexUrl) {
      logger.error("get url failed: ", res.text);
      throw {errMsg: "empty url"};
    }
  ...
  async getDistrictList() {
    const res = await superagent.get(this.getProjectUrl()).timeout(10000);
  ...
  async getHouseList(projectId, buildingId) {
    ...
    let tdDomList = $('#MainContent_gvxml td', res.text);
    for (let i=0; i<tdDomList.length; i++) {
      ...
      houseList.push({
        identifier: `${projectId}.${buildingId}.${houseSN}`,
        houseSN: houseSN,
